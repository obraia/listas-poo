...
namespace Ex72
{
    class Program
    {
        const float JurosP = 0.005f; // taxa de juros poupança ao mês em porcentagem
        ...
        static int Cont = ContadorDeContas(); // contador de contas cadastradas 
        ...
        static int ContadorDeContas()
        {
            ...
            {
                using (StreamReader sr = File.OpenText(arquivoContador)) // instânciando StreamReader e já pasando o caminho
                {
            ...
            {
                StreamWriter criarArquivoCont = new StreamWriter(arquivoContador); // caso o arquivo não existir
                criarArquivoCont.WriteLine(0);
                criarArquivoCont.Close();
                return 0;
            }
            return contador;
        }
        ...
        static void AtualizarArquivos(int agencia)
        {
            ...
            StreamWriter atualizarCont = new StreamWriter(arquivoContador); // limpar arquivo antes de reescrever dados
            atualizarCont.WriteLine(Cont);
            atualizarCont.Close();
